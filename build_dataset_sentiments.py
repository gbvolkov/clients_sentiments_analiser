...
def build_dataset(
    ...
    df = pd.read_csv(src, encoding="utf-8", header=0)
    data = (
        df
    ...
    # ── Попытка стратифицированного деления
    try:
        ...
        print("[warn] Stratify не удался:", e)
        print("[warn] Перехожу на обычное случайное деление без stratify.")
        train_df, temp_df = train_test_split(
            data,
            test_size=dev_ratio + test_ratio,
            shuffle=True,
    ...
    for name, split in [("train", train_df), ("valid", dev_df), ("test", test_df)]:
        split.to_csv(out_dir / f"{name}.csv", index=False, encoding="utf-8")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--src", type=Path, default=Path("data/dataset.csv"), help="Путь к исходному CSV/XLSX")
    parser.add_argument("-o", "--out", type=Path, default=Path("data/sentiment"),
